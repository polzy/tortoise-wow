...
static void Mangosd_WriteCrashDump(EXCEPTION_POINTERS* ep, DWORD synthCode, const char* tag)
{
    ...
    if (FILE* tf = fopen(txtFilename, "w"))
    {
        ...
        // Stack walk — find what called what at the moment of crash. Without this we only
        // have the IP (usually deep in ntdll heap manager), useless for finding which playerbot
        // code corrupted the heap. With this we get the full chain from the corrupting alloc
        // back to our code. Resolve addresses + module + offset; the user can match against
        // the .pdb shipped next to mangosd.exe with link.exe /dump or VS to get line numbers.
        __try
        {
            ...
            // SPECIAL CASE: null function pointer call (IP == 0).
            // When code does `obj->virt()` on an object with a corrupted vtable, or
            // `funcPtr()` on a null function pointer, the CPU jumps to address 0 and
            // crashes. The return address from the CALL was already pushed to *RSP,
            // so we look it up explicitly — otherwise the walker would terminate at
            // the very first frame because PC==0. This is the canonical signature of
            // a use-after-free of a polymorphic object: vtable read returned 0, jmp 0,
            // boom. The .txt previously showed empty stack traces for these.
#ifdef _M_X64
            if (ctx.Rip == 0)
            {
                DWORD64 savedRet = 0;
                __try { savedRet = *(DWORD64*)ctx.Rsp; }
                __except(EXCEPTION_EXECUTE_HANDLER) {}
                fprintf(tf, "[NULL-PC] CALL through 0x0 — caller saved return = 0x%016llx\n",
                        (unsigned long long)savedRet);
                if (savedRet != 0)
                {
                    // Re-seed the walker at the caller so we can unwind from there.
                    frame.AddrPC.Offset = savedRet;
                    ctx.Rip = savedRet;
                    // The CALL pushed 8 bytes; pop them so RSP points at the caller's frame.
                    frame.AddrStack.Offset = ctx.Rsp + 8;
                    ctx.Rsp += 8;
                }
            }
#endif
            ...
            for (int i = 0; i < 40; ++i)
            {
                ...
                const char* symName = "?";
                if (SymFromAddr(GetCurrentProcess(), frame.AddrPC.Offset, &displacement, sym))
                    symName = sym->Name;
                ...
                const char* modName = "?";
                if (SymGetModuleInfo64(GetCurrentProcess(), frame.AddrPC.Offset, &mod))
                ...
                const char* fileName = "";
                DWORD lineNum = 0;
                if (SymGetLineFromAddr64(GetCurrentProcess(), frame.AddrPC.Offset, &lineDisp, &line))
                {
                    fileName = line.FileName ? line.FileName : "";
                    lineNum = line.LineNumber;
                }

                fprintf(tf, "[%02d] 0x%016llx  %s!%s+0x%llx  (%s:%lu)\n",
                        i, (unsigned long long)frame.AddrPC.Offset,
                        modName, symName, (unsigned long long)displacement,
        ...
        __except(EXCEPTION_EXECUTE_HANDLER)
        {
            fprintf(tf, "\n[STACK TRACE FAILED — exception during walk]\n");
        }

...
public:
    ...
    std::list<std::string> HandleRevive(Player* master, const std::string param, AccountTypes security);
protected:
...
private:
    ...
    std::string HandleBotRole(Player* bot, Player* master, const std::string param);
    ...
    std::string HandleBotRevive(Player* bot, Player* master, const std::string param);
    std::string HandleBotBis(Player* bot, Player* master, const std::string param);
    std::string HandleBotFR(Player* bot, Player* master, const std::string param);
    std::string HandleBotResist(Player* bot, Player* master, const std::string param);
    // Per-school aliases — `.bot resist <school> *` fails because the dispatcher
    // consumes "<school>" as the bot name. These are bound to .bot fr / frostres
    // / natres / shadowres so the addon can target with the standard `*` wildcard.
    std::string HandleBotFrostRes(Player* bot, Player* master, const std::string param);
    std::string HandleBotNatRes(Player* bot, Player* master, const std::string param);
    std::string HandleBotShadowRes(Player* bot, Player* master, const std::string param);
    std::string HandleBotInspectGear(Player* bot, Player* master, const std::string param);
...
public:
    // Queue for adding bots with throttling (1 per second)
    std::vector<uint32> m_pendingGroupBotGuids;
    time_t m_nextBotAddTime = 0;
};

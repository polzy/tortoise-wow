...
public:    
    ...
    static bool IsCastInterruptible(Unit* target);
...
public:
    ...
    // Cooldown timestamp (seconds since epoch) for the auto-teleport-to-master
    // check that fires when the bot and master are on different maps.  Prevents
    // spamming TeleportTo every tick while the previous teleport is still being
    // processed or the master is mid-zone-transition.  Cleared to 0 on
    // successful teleport scheduling; set to now+N after each attempt.
    time_t scboteAutoTeleportCooldown = 0;

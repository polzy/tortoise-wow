...
static volatile uint32 s_updatePhase = 0;
uint32 RandomPlayerbotMgr_GetCrashPhase() { return s_updatePhase; }
...
void RandomPlayerbotMgr::UpdateAIInternal(uint32 elapsed, bool minimal)
{
    ...
    s_updatePhase = 10;
    static uint32 updateAIDbgCounter = 0;
    if (++updateAIDbgCounter <= 3)
        sLog.outString("[PB_TICK] UpdateAIInternal completed tick #%u OK", updateAIDbgCounter);
}
...
{
    // Use REPLACE INTO instead of DELETE+INSERT to avoid deadlocks on InnoDB.
    // Requires UNIQUE index (owner, bot, event) on ai_playerbot_random_bots.
    if (value)
    {
        if (data != "")
        {
            CharacterDatabase.PExecute(
                "REPLACE INTO ai_playerbot_random_bots (owner, bot, `time`, validIn, event, `value`, `data`) VALUES ('%u', '%u', '%u', '%u', '%s', '%u', '%s')",
                0, bot, (uint32)time(0), validIn, event.c_str(), value, data.c_str());
        }
        else
        {
            CharacterDatabase.PExecute(
                "REPLACE INTO ai_playerbot_random_bots (owner, bot, `time`, validIn, event, `value`, `data`) VALUES ('%u', '%u', '%u', '%u', '%s', '%u', '')",

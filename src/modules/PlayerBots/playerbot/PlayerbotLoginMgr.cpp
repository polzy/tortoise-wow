...
bool PlayerLoginInfo::LoginBot()
{
    ...
    if (sObjectMgr.GetPlayer(ObjectGuid(HIGHGUID_PLAYER, guid), false))
    {
        loginState = LoginState::BOT_ONLINE;
        holderState = HolderState::HOLDER_EMPTY;
        if (holder) { delete holder; holder = nullptr; }
        return false;
    }

    // Perform the login directly instead of delegating to
    // PlayerbotHolder::HandlePlayerBotLoginCallback, which requires the
    // holder to be registered in m_pendingBotLogins (only done by the
    // synchronous AddPlayerBot path). The async path creates holders in
    // SendHolder() without registering them, so the callback silently
    // returned without logging in any bots.
    LoginQueryHolder* lqh = static_cast<LoginQueryHolder*>(holder);

    WorldSession* botSession = new WorldSession(lqh->GetAccountId(),
        /*sock*/ nullptr, SEC_PLAYER,
        /*mute_time*/ 0, LOCALE_enUS,
        /*remote_ip*/ "disconnected/bot",
        /*binaryIp*/ 0);
    botSession->SetNoAnticheat();
    botSession->SetPlayerLoading(true);

    // HandlePlayerLogin owns the holder from this point.
    ...
    {
        sLog.outError("[AsyncBotLogin] LoginBot: bot %u failed to enter world", guid);
        // Clean up the orphaned session to avoid memory leaks.
        if (!player)
            delete botSession;
        loginState = LoginState::BOT_OFFLINE;
        return false;
    ...
    sLog.outString("[AsyncBotLogin] Bot %s (guid %u) entered world at map %u",
                   player->GetName(), guid, player->GetMapId());
...
void PlayerBotLoginMgr::Update(RealPlayers& realPlayers)
{
    ...
    if (botPool.empty())
    {
        ...
        if (debug)
            sLog.outError("[AsyncBotLogin] Update: botPool empty, waiting for DB load...");
        return;
    }

    if (debug)
        sLog.outError("[AsyncBotLogin] Update: botPool size=%u, realPlayers=%u, onlineBots=%u",
                       (uint32)botPool.size(), (uint32)realPlayers.size(), (uint32)onlineBots.size());

    BotInfos queue = GetFuture(FillLoginLogoutQueue, futureQueue, true, &botPool, realPlayers);

    if (debug)
        sLog.outError("[AsyncBotLogin] Update: queue returned %u entries", (uint32)queue.size());
...
{
    uint32 loginOk = 0, loginFail = 0, logoutOk = 0;
    for (auto& info : queue)
    {
        if (info->LoginBot())
        {
        ...
        else if (info->GetLoginState() == LoginState::BOT_ON_LOGINQUEUE)
        {
            loginFail++;
        }
        if (info->LogoutBot())
    ...
    if (debug && (loginOk || loginFail || logoutOk))
        sLog.outError("[AsyncBotLogin] LoginLogoutBots: logged in %u, pending holder %u, logged out %u",
                       loginOk, loginFail, logoutOk);
}

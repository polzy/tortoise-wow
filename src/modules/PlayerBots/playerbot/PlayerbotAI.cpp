...
{
    // Remove from Engine's ready/broken sets so a later OnBotLogin that reuses this
    // pointer (heap reuse) doesn't see stale ready/broken state.
    extern void Engine_MarkAIDestroyed(PlayerbotAI* ai);
    Engine_MarkAIDestroyed(this);
...
extern "C" volatile uint32 s_botAIPhase = 0;
static const char* s_botAICrashName = "";
uint32 PlayerbotAI_GetCrashPhase() { return s_botAIPhase; }
const char* PlayerbotAI_GetCrashName() { return s_botAICrashName; }

void PlayerbotAI::UpdateAI(uint32 elapsed, bool minimal)
{
    if (!bot || !bot->IsInWorld())
        return;
    if (!aiObjectContext || !reactionEngine)
        return;

    s_botAICrashName = bot->GetName();
    s_botAIPhase = 1;
    ...
    // --- Auto-teleport to master on map change (instance enter/exit) ---
    // When the master moves to a different map (enters or leaves a dungeon,
    // crosses a continent portal, etc.) the bot is stranded on the old map
    // because there is no client to walk through the portal.  Detect the
    // map mismatch and schedule a TeleportTo so the bot follows.
    //
    // Guards:
    //   * master must exist, be in-world, and NOT mid-teleport itself
    //     (if master is still teleporting the destination coords are stale)
    //   * bot must not already be mid-teleport
    //   * bot must not be in a battleground (BG has its own teleport logic)
    //   * bot must not be taxi-flying (can't interrupt flight paths)
    //   * cooldown: at most one attempt every 5 seconds so we don't spam
    //     TeleportTo while the previous one is still being ACK'd
    //   * only for bots with a real-player master (not random bots wandering)
    ...
    {
        time_t now = time(0);
        if (now >= scboteAutoTeleportCooldown)
        {
            scboteAutoTeleportCooldown = now + 30;

            sLog.outDetail("[PlayerBots] Auto-teleport: %s (map %u) -> master %s (map %u pos %.1f,%.1f,%.1f)",
                bot->GetName(), bot->GetMapId(),
                master->GetName(), master->GetMapId(),
            ...
            // Don't run any further AI this tick — the teleport is in
            // flight and the bot's world-state is about to change.
            SetAIInternalUpdateDelay(urand(2000, 4000));
            return;
        }
    }
...
void PlayerbotAI::DoNextAction(bool min)
{
	...
	{
		// Never walk during combat -- the bot must always run to chase/position
		if (!sServerFacade.IsInCombat(bot) && master->m_movementInfo.HasMovementFlag(MOVEFLAG_WALK_MODE) && sServerFacade.GetDistance2d(bot, master) < 20.0f) bot->m_movementInfo.AddMovementFlag(MOVEFLAG_WALK_MODE);
...
ActivePiorityType PlayerbotAI::GetPriorityType()
{
    ...
    Map* map = bot->GetMap();
    if (bot->IsBeingTeleported() || !bot->IsInWorld() || !map || !map->HasRealPlayers())
        return ActivePiorityType::IN_INACTIVE_MAP;

    if (!map->HasActiveZone(bot->GetZoneId()))
...
bool PlayerbotAI::AddAura(Unit* unit, uint32 spellId)
{
    ...
    // CRITICAL: Unit::AddSpellAuraHolder DELETES the holder itself on every false-return path
    // (dead target, mismatched target, refresh-existing, stack-existing, rank-conflict, etc.).
    // The original `if (!...) delete holder;` here was a guaranteed double-free that produced
    // the heap-corruption 0xc0000374 — observed via stack walk crash_20260528_115156.dmp.txt
    // showing free_base → AddAura+0xe1 → DrinkAction::Execute. We must NOT delete here.

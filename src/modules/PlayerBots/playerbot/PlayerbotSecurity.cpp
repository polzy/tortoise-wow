...
// `from` may be a dangling Player* (chat command processing races vs the player Player
// object being destroyed mid-iteration). Reading any field triggers an AV at the field's
// offset from 0 — we observed READ at 0x330 (= offset of m_accountId in WorldSession) in
// three crashes. Can't reliably null-check a dangling pointer, so we contain the failure
// with __try/__except. __declspec(noinline) forces the SEH frame to actually exist at
// call time so the compiler doesn't elide it back into LevelFor.
__declspec(noinline) static int SafeProbeFrom(Player* from, uint32 expectedAccount, int* outIsGm)
{
    *outIsGm = 0;
    __try
    {
        if (!from) return -1;
        WorldSession* sess = from->GetSession();
        if (!sess) return -1;
        if (sess->GetSecurity() >= SEC_GAMEMASTER) { *outIsGm = 1; return 1; }
        return (sess->GetAccountId() == expectedAccount) ? 1 : 0;
    }
...
{
    // The previous version delegated the probe to SafeProbeFrom() with __declspec(noinline)
    // + __try, but LTCG inlined the helper anyway (observed in crash_20260528_155149: AV at
    // LevelFor+0xc with no SafeProbeFrom frame on the stack — read at 0x330 = m_accountId).
    // Inlining the __try block directly into LevelFor is the only way to guarantee the SEH
    // filter is actually around the dereference.
    //
    // Note: __try requires the function have NO C++ objects with destructors active.
    // LevelFor's only locals are POD (int), so this is safe per MSVC spec.
    int isGm = 0;
    int probe = -1;
    __try
    {
        ...
        else
        {
            ...
            if (!sess)
                probe = -1;
            else if (sess->GetSecurity() >= SEC_GAMEMASTER)
            {
                isGm = 1;
                probe = 1;
            }
            else
                probe = (sess->GetAccountId() == account) ? 1 : 0;
        }
    }
    ...
    if (probe < 0)
    {
        if (reason) *reason = DenyReason::PLAYERBOT_DENY_NONE;
        return PlayerbotSecurityLevel::PLAYERBOT_SECURITY_DENY_ALL;
    }
    if (probe == 1)
        return PlayerbotSecurityLevel::PLAYERBOT_SECURITY_ALLOW_ALL;

    // Phase 2 — `from` is valid but not GM and not on the bot's account. For a private
    // MC raid the simplest correct behavior is INVITE (any group member can command the
    // bot). The original slow-path did gearscore/BG/LFG/level checks that are correct for
    // a public realm but unnecessary here, and they kept producing AVs deep in the body
    // (from->GetGroup() then iterating with the dangling member can still trip). Returning
    // INVITE skips all that risk.
    if (reason) *reason = DenyReason::PLAYERBOT_DENY_INVITE;
    return PlayerbotSecurityLevel::PLAYERBOT_SECURITY_INVITE;
}

#if 0 // entire legacy slow path retained for reference / future re-enable

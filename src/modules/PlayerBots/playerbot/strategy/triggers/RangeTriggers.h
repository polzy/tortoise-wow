...
namespace ai
{
    ...
    public:
        ...
        virtual bool IsActive() override
        {
            ...
            if (target)
            {
                // Original guard: if follow/guard/wander strategy active, suppress the
                // flee trigger so the bot doesn't break formation. That makes sense for
                // melee classes (which should stay in melee anyway) but kills caster
                // survivability — raid bots ALWAYS have "follow" active (RaidFollowDistance
                // = 5) so casters never back off when a mob runs into their face and they
                // get one-shot. The trigger must fire for casters regardless of follow.
                ...
                // Hunter exception preserved from original: don't fire at <=5y (let pet
                // tank up close); the rule above already allows hunters >5y to flee
                // back to ranged.
                if (cls == CLASS_HUNTER && sServerFacade.GetDistance2d(bot, target) <= 5.0f)
                    return false;

...
extern "C" volatile uint32 s_botAIPhase;
static char s_lastActionBuf[64] = "(none)";
const char* Engine_GetLastActionName() { return s_lastActionBuf; }

// Broken-AI quarantine. Each AV in an Action call corrupts the heap a little (the AV
// usually fires mid-allocator-write or mid-Unit-deref, leaving freelist/object metadata
// inconsistent). The SEH wrapper here resumes execution after the AV, but the damage is
// already done — and the same bot keeps tripping the same null deref every tick, adding
// 30-40 corruptions/min. Without quarantine the heap detector trips 0xc0000374 within
// minutes (see crash_20260528_103032: 6 AVs in 10s pre-login → boom 9s after Polz logged in).
//
// On AV we insert the offending PlayerbotAI* into g_brokenAIs. Engine::DoNextAction
// checks this set first thing and bails out, so the broken bot stops ticking entirely.
// The bot stays in the world (no kick — that path itself can AV) but its AI is silent.
// Set lives at file scope so it outlives any AI; entries are never erased (it's small
// and the entries are unique per broken bot per session).
static std::unordered_set<PlayerbotAI*> g_brokenAIs;

bool Engine_IsBrokenAI(PlayerbotAI* ai) { return g_brokenAIs.count(ai) > 0; }

// AIs that have not yet completed OnBotLogin. Their internal state (engines, contexts,
// strategies) is half-built — calling DoNextAction on them triggers AVs because actions
// reference values that don't exist yet. Re-entrancy: the user observed
//   [OBL-PHASE] X: creating PlayerbotAI
//   [BOTAI] Action 'mind flay' crashed in Execute() for X — AI quarantined
//   [OBL-PHASE] X: HasRealPlayerMaster check
// — i.e. an AI tick fired between two phase markers of OnBotLogin. Each such AV is a
// heap-corruption seed, and with 40 bots logging in this hits 0xc0000374 in seconds.
// Engine_MarkAIReady is called at the END of OnBotLogin; until then DoNextAction bails.
static std::unordered_set<PlayerbotAI*> g_readyAIs;
void Engine_MarkAIReady(PlayerbotAI* ai) { if (ai) g_readyAIs.insert(ai); }
void Engine_MarkAIDestroyed(PlayerbotAI* ai) { if (ai) { g_readyAIs.erase(ai); g_brokenAIs.erase(ai); } }
bool Engine_IsAIReady(PlayerbotAI* ai) { return g_readyAIs.count(ai) > 0; }

static void MarkBrokenAI(PlayerbotAI* ai, const char* where)
{
    const char* botName = "?";
    if (ai)
    {
        g_brokenAIs.insert(ai);
        if (ai->GetBot())
            botName = ai->GetBot()->GetName();
    }
    sLog.outError("[BOTAI] Action '%s' crashed in %s() for bot '%s' — AI quarantined to stop heap corruption",
                  s_lastActionBuf, where, botName);
}

static bool SafeIsUseful(Action* action, PlayerbotAI* ai)
{
    __try { return action->isUseful(); }
    __except(EXCEPTION_EXECUTE_HANDLER) {
        MarkBrokenAI(ai, "isUseful");
        return false;
    }
}

static bool SafeExecute(Action* action, Event& event, PlayerbotAI* ai)
{
    __try { return action->Execute(event); }
    __except(EXCEPTION_EXECUTE_HANDLER) {
        MarkBrokenAI(ai, "Execute");
        return false;
    }
}
...
{
    // Quarantine: if any Action AVed on a previous tick for this AI, skip all work.
    // The bot stays in-world but its AI is silent. Prevents repeated AVs from the same
    // broken state pile-on-heap-corruption.
    if (g_brokenAIs.count(ai))
        return false;

    // NOT YET INITIALIZED guard: until OnBotLogin completes, the AI's context/strategies/
    // listeners may be half-constructed. Running actions against them produces AVs that
    // corrupt the heap. OnBotLogin calls Engine_MarkAIReady at the end of its setup; we
    // stay silent until then. This eliminates the re-entrancy-induced 0xc0000374 we
    // were chasing for hours (observed pattern: action crash log appearing BETWEEN two
    // OBL-PHASE markers of the same bot's OnBotLogin).
    if (g_readyAIs.count(ai) == 0)
        return false;
    ...
    do 
    {
        ...
        if (basket) 
        {
            ...
            s_botAIPhase = 9333;
            {
                const std::string& aname = actionNode->getName();
                strncpy(s_lastActionBuf, aname.c_str(), sizeof(s_lastActionBuf) - 1);
                s_lastActionBuf[sizeof(s_lastActionBuf) - 1] = '\0';
            }
            s_botAIPhase = 9334;
            ...
            else
            {
                ...
                if (!isStunned || action->isUsefulWhenStunned())
                {
                    ...
                    isUseful = SafeIsUseful(action, ai);
                    pmo2.reset();
                }
...
{
    // Same ready/broken gates as DoNextAction. DoSpecificAction (and the PlayerbotAI
    // constructor's "auto talents" call line 241) reach ExecuteAction WITHOUT going through
    // DoNextAction, so the DoNextAction gate alone isn't enough. Without this, an action
    // can run on a half-built AI (engines/context not populated yet) and AV → heap corruption.
    if (g_brokenAIs.count(ai) || g_readyAIs.count(ai) == 0)
...
bool Engine::ListenAndExecute(Action* action, Event& event)
{
    ...
    if (actionExecutionListeners.Before(action, event))
    {
        ...
        actionExecuted = actionExecutionListeners.AllowExecution(action, event) ? SafeExecute(action, event, ai) : true;
        if (actionExecuted)
        {

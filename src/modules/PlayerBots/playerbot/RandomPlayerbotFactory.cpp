...
bool RandomPlayerbotFactory::CreateRandomBot(uint8 cls, uint8 inputRace)
{
    ...
    sLog.outString("[RNDBOT] Creating random bot: class=%u inputRace=%u account=%u", cls, inputRace, accountId);

    uint8 gender = urand(0, 1) ? GENDER_MALE : GENDER_FEMALE;

    uint8 race = inputRace == 0 ? GetRandomRace(cls) : inputRace;
    sLog.outString("[RNDBOT] Picked race=%u gender=%u for class=%u", race, gender, cls);
    ...
    if (skinColors.empty()) skinColors.push_back(urand(0, 5));
    if (faces.empty()) faces.push_back({urand(0, 7), urand(0, 5)});
    if (hairs.empty()) hairs.push_back({urand(0, 11), urand(0, 5)});
    if (facialHairTypes.empty()) facialHairTypes.push_back(urand(0, 5));
    ...
    sLog.outString("[RNDBOT] SUCCESS: bot created for account %d - name: \"%s\"; race: %u; class: %u",
            accountId, name.c_str(), race, cls);
...
{
    sLog.outString("[DEBUG-BOTS] CreateRandomBots: calling EnsureNamesInitialized...");
    EnsureNamesInitialized();
    sLog.outString("[DEBUG-BOTS] CreateRandomBots: names initialized, checking bot_delete...");
    ...
    sLog.outString("[DEBUG-BOTS] CreateRandomBots: delAccs=%u, deleteRandomBotAccounts=%u", delAccs, sPlayerbotAIConfig.deleteRandomBotAccounts);
    ...
    sLog.outString("[DEBUG-BOTS] CreateRandomBots: checking temporary bots...");
    ...
    sLog.outString("[DEBUG-BOTS] CreateRandomBots: autoCreate=%u, accountCount=%u", sPlayerbotAIConfig.randomBotAutoCreate, sPlayerbotAIConfig.randomBotAccountCount);
	...
	sLog.outString("[RNDBOT] Account %s (id=%u) has %d chars, creating bots...", accountName.c_str(), accountId, count);

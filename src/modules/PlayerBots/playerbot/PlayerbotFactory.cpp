...
{
    sLog.outString("[PB_FACTORY] Init() start");
    if (sPlayerbotAIConfig.randomBotPreQuests) {
        sLog.outString("[PB_FACTORY] Loading pre-quests...");
        ...
        sLog.outString("[PB_FACTORY] Pre-quests loaded, loading special quests...");
        ...
        sLog.outString("[PB_FACTORY] Special quests loaded.");
    }

    sLog.outString("[PB_FACTORY] Loading taxi nodes...");
    ...
    sLog.outString("[PB_FACTORY] Init() done");
}
...
void PlayerbotFactory::InitEquipment(bool incremental, bool syncWithMaster, bool progressive, bool partialUpgrade)
{
    ...
    // NOTE: BiS gear equip removed from here — the StoreNewItem/EquipItem sequence at the
    // top of InitEquipment interfered with the random gear path below (DestroyItem on a
    // slot that had just been mutated produced heap corruption — crash_20260528_150533).
    // BiS application moved to a dedicated call after InitEquipment finishes; see
    // ApplyBiSGear() invoked from Randomize() / Refresh() once the random path is done.
        ...
        std::sort(itemIds.begin(), itemIds.end(), [specId](int a, int b)
            {
                ...
                if (!proto1 || !proto2) return proto2 != nullptr;
    ...
    for(uint8 slot = 0; slot < EQUIPMENT_SLOT_END; ++slot)
    {
                    ...
                    std::sort(ids.begin(), ids.end(), [specId](int a, int b)
                        {
                            ...
                            if (proto1) baseCompareA += proto1->Quality * proto1->ItemLevel;
                            if (proto2) baseCompareB += proto2->Quality * proto2->ItemLevel;
        ...
        if (!found)
        {
            if (slot != EQUIPMENT_SLOT_TRINKET1 && slot != EQUIPMENT_SLOT_TRINKET2)
                sLog.outDetail("Bot #%d %s:%d <%s>: no items for slot %d, quality >= %u",
                               bot->GetGUIDLow(), bot->GetTeam() == ALLIANCE ? "A" : "H",
                               bot->GetLevel(), bot->GetName(), slot, quality);
            continue;
        }
    }
...
// do not let skill go beyond limit even if maxlevel > blizzlike
    ...
    // CAPPED at maxValue (was urand(maxValue - level, maxValue) which left bots 60 skill points
    // short of cap on average). For MC raid bots that's critical: level 63 bosses require 305
    // weapon skill (300 cap + 5 racial) to avoid the 6.5%/skill miss/glance penalty. Random
    // values 240-300 meant most bots missed and glanced heavily — they got "demolished".
...
void PlayerbotFactory::InitAvailableSpells()
{
    ...
    // CRITICAL: Penqle's Player.h declares learnDefaultSpells / learnClassLevelSpells as
    // EMPTY STUBS ("cmangos training helpers; Penqle has equivalents" — comment lies, no
    // equivalent is wired in). As a result bots ended up at lvl 60 with 0 class combat
    // spells: no Taunt, no Sunder Armor, no Mortal Strike, no Charge, no Heroic Strike.
    // The 200+ spells they DID have were all profession recipes from a different code
    // path. This made tanks unable to taunt, melees unable to use abilities, etc.
    //
    // Replacement: iterate every SkillLineAbility row whose class_mask includes this bot
    // and whose race_mask matches, then learn the spell if the bot's level is high enough.
    // Mirrors what an NPC trainer's "learn all available" would do.
    bot->learnDefaultSpells();   // no-op stub in Penqle, kept for upstream compatibility
    bot->learnClassLevelSpells(true);  // also a no-op stub
    LearnPenqleClassSpells();
...
// Workaround for Penqle's empty learnDefaultSpells / learnClassLevelSpells stubs
// (see Player.h:2457-2459). Iterates skill_line_ability for every spell whose
// class_mask includes this bot's class and race_mask matches, then calls learnSpell
// for the appropriate rank at the bot's level. Without this, lvl-60 warrior bots
// had ZERO combat abilities (no Taunt, Sunder, Mortal Strike, Charge, Heroic Strike) —
// they only had profession recipes and a handful of manual book-spell pushes.
//
// We rely on the in-memory SkillLineAbility map exposed by sSpellMgr; iterating the
// whole map once per bot is acceptable cost (~5-10k entries, sub-ms).
void PlayerbotFactory::LearnPenqleClassSpells()
{
    const uint32 classMask = bot->GetClassMask();
    const uint32 raceMask  = bot->GetRaceMask();
    const uint32 botLevel  = bot->GetLevel();

    // Query candidate spells for this class straight from skill_line_ability — we can't
    // reach SpellMgr's private SkillLineAbility map without exposing an accessor, and
    // walking spell IDs 1..MAX would touch ~30k entries per bot. The SQL pull is one
    // call that returns just the rows that could possibly match.
    QueryResult* result = WorldDatabase.PQuery(
        "SELECT spell_id, race_mask FROM skill_line_ability "
        "WHERE class_mask = 0 OR (class_mask & %u) != 0",
        classMask);

    if (!result)
    {
        sLog.outError("[SPELLS] %s: skill_line_ability query returned no rows", bot->GetName());
        return;
    }
    ...
    do
    {
        ...
        uint32 spellRaceMask = fields[1].GetUInt32();
        ++considered;

        if (spellRaceMask && (spellRaceMask & raceMask) == 0)
            continue;

        if (bot->HasSpell(spellId))
            continue;

        // Defer to MaNGOS's own gate for the level/rank chain. IsSpellFitByClassAndRace
        // returns false when reqLevel > bot->GetLevel(); we don't want low-level pre-req
        // ranks of higher spells to be auto-learnt (chain ranks supersede each other).
        if (!bot->IsSpellFitByClassAndRace(spellId))
            continue;

        // Spell prototype existence check — some skill_line_ability rows reference removed
        // or invalid spell IDs in modified DBs.
    ...
    sLog.outString("[SPELLS] %s (cls=%u lvl=%u): learned %u new class spells (out of %u candidates)",
                   bot->GetName(), bot->getClass(), botLevel, learned, considered);
}

...
static bool SafeCall_CreateRandomBots()
{
    __try { RandomPlayerbotFactory::CreateRandomBots(); return true; }
    __except(EXCEPTION_EXECUTE_HANDLER) { return false; }
}
static bool SafeCall_PlayerbotFactoryInit()
{
    __try { PlayerbotFactory::Init(); return true; }
    __except(EXCEPTION_EXECUTE_HANDLER) { return false; }
}
static bool SafeCall_LoadQuestTravelTable()
{
    __try { sTravelMgr.LoadQuestTravelTable(); return true; }
    __except(EXCEPTION_EXECUTE_HANDLER) { return false; }
}
static bool SafeCall_LoadNamedLocations()
{
    __try { sRandomPlayerbotMgr.LoadNamedLocations(); return true; }
    __except(EXCEPTION_EXECUTE_HANDLER) { return false; }
}
static bool SafeCall_LoadBattleMastersCache()
{
    __try { sRandomPlayerbotMgr.LoadBattleMastersCache(); return true; }
    __except(EXCEPTION_EXECUTE_HANDLER) { return false; }
}
static bool SafeCall_RandomItemMgrInit()
{
    __try { sRandomItemMgr.Init(); return true; }
    __except(EXCEPTION_EXECUTE_HANDLER) { return false; }
}
static bool SafeCall_LoadBotTexts()
{
    __try { sPlayerbotTextMgr.LoadBotTexts(); sPlayerbotTextMgr.LoadBotTextChance(); return true; }
    __except(EXCEPTION_EXECUTE_HANDLER) { return false; }
}
static bool SafeCall_LoadBotHelpTexts()
{
    __try { sPlayerbotHelpMgr.LoadBotHelpTexts(); return true; }
    __except(EXCEPTION_EXECUTE_HANDLER) { return false; }
}
// LoadTalentSpecs is private, called inline from Initialize()
...
bool PlayerbotAIConfig::Initialize()
{
    ...
    sLog.outString("[PB_INIT] checkpoint A: enabled=true");

    ConfigAccess* configA = reinterpret_cast<ConfigAccess*>(&config);
    sLog.outString("[PB_INIT] checkpoint B: configA cast done");

    BarGoLink::SetOutputState(config.GetBoolDefault("AiPlayerbot.ShowProgressBars", false));
    sLog.outString("[PB_INIT] checkpoint C: BarGoLink done");
    ...
    sLog.outString("[PB_INIT] checkpoint D: basic config done");
    ...
    sLog.outString("[PB_INIT] checkpoint E: loading lists...");
    ...
    sLog.outString("[PB_DEBUG] autoCreate=%u, autologin=%u, minBots=%u, maxBots=%u, botAutologin=%u",
        (uint32)randomBotAutoCreate, (uint32)randomBotAutologin, minRandomBots, maxRandomBots, (uint32)botAutologin);
    ...
    sLog.outString("[PB_INIT] checkpoint F: before Race/Class probabilities");
    sLog.outString("Loading Race/Class probabilities");

    classRaceProbabilityTotal = 0;
    sLog.outString("[PB_INIT] checkpoint G: before RandomPlayerbotFactory");

    useFixedClassRaceCounts = config.GetBoolDefault("AiPlayerbot.ClassRace.UseFixedClassRaceCounts", false);
    sLog.outString("[PB_INIT] checkpoint G1: creating factory...");
    RandomPlayerbotFactory factory(0);
    sLog.outString("[PB_INIT] checkpoint G2: factory created OK");
    ...
    sLog.outString("[PB_INIT] checkpoint H: race/class loop done, total=%u", classRaceProbabilityTotal);
    ...
    sLog.outString("[PB_INIT] checkpoint I: before cheat mask");
    ...
    sLog.outString("[PB_INIT] checkpoint J: before randomBotAccount config");
    ...
    sLog.outString("[PB_INIT] checkpoint K: before guild/broadcast config");
    ...
    sLog.outString("[PB_INIT] checkpoint L: before LLM prompts file load");
    {
    ...
    sLog.outString("[PB_INIT] checkpoint M: before gear progression system");
    ...
    sLog.outString("[PB_INIT] checkpoint N: before loading free bots");
    ...
    sLog.outString("[PB_INIT] checkpoint O: before loadFreeAltBotAccounts");
    loadFreeAltBotAccounts();

    targetPosRecalcDistance = config.GetFloatDefault("AiPlayerbot.TargetPosRecalcDistance", 0.1f),

    sLog.outString("[PB_INIT] checkpoint P: before LoadAreaLevels/PopulateSpells");
    ...
    sLog.outString("[PB_INIT] checkpoint Q: before CreateRandomBots");
    if (!SafeCall_CreateRandomBots())
        sLog.outError("[PB_INIT] CreateRandomBots CRASHED - skipping");

    sLog.outString("[PB_INIT] checkpoint Q2: before PlayerbotFactory::Init");
    if (!SafeCall_PlayerbotFactoryInit())
        sLog.outError("[PB_INIT] PlayerbotFactory::Init CRASHED - skipping (taxi nodes)");
    sLog.outString("[PB_INIT] checkpoint Q3: before sRandomItemMgr.Init");
    if (!SafeCall_RandomItemMgrInit())
        sLog.outError("[PB_INIT] sRandomItemMgr.Init() CRASHED - skipping");

    sLog.outString("[PB_INIT] checkpoint Q4: before LoadBotTexts");
    if (!SafeCall_LoadBotTexts())
        sLog.outError("[PB_INIT] LoadBotTexts CRASHED - skipping");

    sLog.outString("[PB_INIT] checkpoint Q5: before LoadBotHelpTexts");
    if (!SafeCall_LoadBotHelpTexts())
        sLog.outError("[PB_INIT] LoadBotHelpTexts CRASHED - skipping");

    sLog.outString("[PB_INIT] checkpoint R: before LoadTalentSpecs");
    try { LoadTalentSpecs(); }
    catch (...) { sLog.outError("[PB_INIT] LoadTalentSpecs threw exception - skipping"); }

    sLog.outString("[PB_INIT] checkpoint S: before LoadQuestTravelTable");
    if (sPlayerbotAIConfig.autoDoQuests)
    {
        sLog.outString("Loading Quest Detail Data...");
        if (!SafeCall_LoadQuestTravelTable())
            sLog.outError("[PB_INIT] LoadQuestTravelTable CRASHED - travel data unavailable but bots will still work");
        else
            sLog.outString("[PB_INIT] LoadQuestTravelTable completed OK");
    }

    sLog.outString("[PB_INIT] checkpoint T: before LoadNamedLocations");
    sLog.outString("Loading named locations...");
    if (!SafeCall_LoadNamedLocations())
        sLog.outError("[PB_INIT] LoadNamedLocations CRASHED - skipping");

    sLog.outString("[PB_INIT] checkpoint U: before LoadBattleMastersCache");
    if (sPlayerbotAIConfig.randomBotJoinBG)
    {
        if (!SafeCall_LoadBattleMastersCache())
            sLog.outError("[PB_INIT] LoadBattleMastersCache CRASHED - skipping");
    }

    // Clear randomize events for bots that need gear so ProcessBot initializes them
    if (randomBotAutoCreate)
    {
        CharacterDatabase.DirectExecute(
            "DELETE FROM ai_playerbot_random_bots WHERE event = 'randomize' "
            "AND bot IN (SELECT guid FROM characters WHERE totaltime = 0 AND level >= 2)");
        sLog.outString("Cleared randomize events for uninitiated bots - ProcessBot will gear them gradually");
    }

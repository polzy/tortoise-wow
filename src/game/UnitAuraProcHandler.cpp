...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(Unit* pVictim, uint32 damage, int32 /*originalAmount*/, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    if (!triggerEntry)
    {
        // Not cast unknown spell. Downgraded from outError because for vanilla dummy auras
        // (e.g. Druid Furor 19556 which uses a runtime computed proc) this is the normal
        // path — the proc handler is supposed to no-op and let other systems handle it.
        // Logging at error level produced thousands of lines in errors.log per raid fight.
        sLog.outDetail("Unit::HandleProcTriggerSpell: Spell %u have %u in EffectTriggered[%d], not handled custom case",
                       auraSpellInfo->Id, trigger_spell_id, triggeredByAura->GetEffIndex());
        return SPELL_AURA_PROC_FAILED;
    }
